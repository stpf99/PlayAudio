...
class AudioPlayer(Gtk.Window):
    def __init__(self):
        ...
        self.album_cover.set_size_request(250, 250)  # Ustawienie żądanych wymiarów obrazka

        grid = Gtk.Grid()
        grid.set_row_homogeneous(True)
        grid.set_column_homogeneous(True)
        grid.attach(self.info_label, 0, 0, 1, 1)
        grid.attach(self.vinyl_image, 1, 0, 1, 1)
        grid.attach(self.album_cover, 2, 0, 1, 1)
    ...
    def on_row_activated(self, treeview, path, column):
        ...
        if tag_info:
            ...
            if album_cover_path:
                album_cover_pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(album_cover_path, 250, 250)  # Pomniejszenie obrazka
    ...
    def rotate_vinyl(self):
        ...
        while not self.rotation_stop:
            ...
            pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_size(rotated_image, 300, 300)  # Przekształcamy obrócony obrazek do formatu GdkPixbuf    
            ...
            time.sleep(0.01)  # Czekamy krótki czas, aby uzyskać płynną animację
