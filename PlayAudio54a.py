...
class MusicPlayer:
    def __init__(self):
        ...
        # Create the main vertical container (vbox)
        vbox = Gtk.VBox(homogeneous=False, spacing=0)  # Initialize VBox first

        # Create an HPaned container to split the window horizontally
        self.hpaned = Gtk.HPaned()

        # Modify now playing layout to be in one row
        hbox_playing_info = Gtk.HBox(homogeneous=False, spacing=10)
        hbox_playing_info.pack_start(self.previous_playing_label, True, True, 2)
        hbox_playing_info.pack_start(self.now_playing_label, True, True, 2)
        hbox_playing_info.pack_start(self.next_playing_label, True, True, 2)
        vbox.pack_start(hbox_playing_info, False, False, 2)  # Now vbox is defined
        ...
        # Replace text buttons with icon buttons and tooltips
        self.play_button = Gtk.Button()
        self.play_button.add(Gtk.Image.new_from_icon_name("media-playback-start", Gtk.IconSize.BUTTON))
        self.play_button.set_tooltip_text("Play")
        self.play_button.connect("clicked", self.play)

        self.pause_button = Gtk.Button()
        self.pause_button.add(Gtk.Image.new_from_icon_name("media-playback-pause", Gtk.IconSize.BUTTON))
        self.pause_button.set_tooltip_text("Pause")
        self.pause_button.connect("clicked", self.toggle_pause)

        self.stop_button = Gtk.Button()
        self.stop_button.add(Gtk.Image.new_from_icon_name("media-playback-stop", Gtk.IconSize.BUTTON))
        self.stop_button.set_tooltip_text("Stop")
        self.stop_button.connect("clicked", self.stop)

        self.next_button = Gtk.Button()
        self.next_button.add(Gtk.Image.new_from_icon_name("media-skip-forward", Gtk.IconSize.BUTTON))
        ...
        self.previous_button.add(Gtk.Image.new_from_icon_name("media-skip-backward", Gtk.IconSize.BUTTON))
...
#    def visualize(self, widget, cr):
        if self.visualizing:
            ...
            visualization_type = {
                "None": self.draw_0,
                "Spectrum": lambda: self.draw_generic(widget, cr, type='spectrum'),
                "Waveform": lambda: self.draw_generic(widget, cr, type='waveform'),
                "Points": lambda: self.draw_generic(widget, cr, type='points'),
                "Tonal Transition": lambda: self.draw_generic(widget, cr, type='tonal_transition')
            }
            visualization_type.get(selected_text, self.draw_0)()

#    def draw_generic(self, widget, cr, type='spectrum'):
        # Consolidate the drawing logic based on the 'type'
        if self.fft_data is not None:
            ...
            for i in range(num_bars):
                bar_height = self.get_bar_height(i, max_amplitude)
                color, transparency = np.random.rand(3), 0.5 + (bar_height * 0.5)
                if type == 'spectrum':
                    self.draw_bar(cr, i * bar_width, (1 - bar_height) * widget.get_allocated_height(), bar_width, bar_height * widget.get_allocated_height(), color, transparency)
                elif type == 'waveform':
                    self.draw_waveform(cr, i * bar_width, bar_height)
                elif type == 'points':
                    self.draw_points(cr, i * bar_width, bar_height)
                elif type == 'tonal_transition':
                    self.draw_tonal_transition(cr, i * bar_width, bar_height)


#    def get_bar_height(self, i, max_amplitude):
        start, end = int((i / 64) * len(self.fft_data)), int(((i + 1) / 64) * len(self.fft_data))
        bar_height = np.max(self.fft_data[start:end]) / max_amplitude if max_amplitude != 0 else 0
        return bar_height
    ...
    def visualize(self, widget, cr):
        if self.visualizing:
            ...
            num_bars = 64  # Adjust bar count if necessary
            bar_width = widget.get_allocated_width() / num_bars
            max_amplitude = np.max(self.fft_data) if self.fft_data is not None else 0

            for i in range(num_bars):
                start, end = int(i / num_bars * len(self.fft_data)), int((i + 1) / num_bars * len(self.fft_data))
                bar_height = np.max(self.fft_data[start:end]) / max_amplitude if max_amplitude != 0 else 0
                color, transparency = np.random.rand(3), 0.5 + (bar_height * 0.5)

                if selected_text == "Spectrum":
                    self.draw_bar(cr, i * bar_width, widget.get_allocated_height(), bar_width, bar_height, color, transparency)
                elif selected_text == "Waveform":
                    self.draw_waveform(cr, i * bar_width, widget.get_allocated_height(), bar_width, bar_height, color)
                elif selected_text == "Points":
                    self.draw_points(cr, i * bar_width, widget.get_allocated_height(), bar_width, bar_height, color, transparency)
                elif selected_text == "Tonal Transition":
                    self.draw_tonal_transition(cr, i * bar_width, widget.get_allocated_height(), bar_width, bar_height, color, transparency)

    def draw_bar(self, cr, x, height, width, bar_height, color, transparency):
        cr.set_source_rgba(*color, transparency)
        cr.rectangle(x, height - bar_height * height, width / 2, bar_height * height)
        cr.fill()

    def draw_points(self, cr, x, height, width, bar_height, color, transparency):
        num_points = max(int(bar_height * 10), 1)
        step = width / num_points
        for i in range(num_points):
            x_pos = x + i * step
            y_pos = height - bar_height * i / num_points * height
            cr.set_source_rgba(*color, transparency)
            cr.arc(x_pos, y_pos, 5, 0, 2 * np.pi)
            cr.fill()

    def draw_waveform(self, cr, x, height, width, bar_height, color):
        num_points = int(width)
        step = width / num_points
        cr.set_source_rgb(*color)
        cr.set_line_width(2)
        for i in range(num_points):
            x1, y1 = x + i * step, height / 2 * np.sin(2 * np.pi * i / num_points)
            x2, y2 = x + (i + 1) * step, height / 2 * np.sin(2 * np.pi * (i + 1) / num_points)
            cr.move_to(x1, height - y1 * bar_height)
            cr.line_to(x2, height - y2 * bar_height)
        cr.stroke()

    def draw_tonal_transition(self, cr, x, height, width, bar_height, color, transparency):
        num_points = int(width)
        step = width / num_points
        cr.set_line_width(2.0)
        for i in range(num_points):
            x1, x2 = x + i * step, x + (i + 1) * step
            gradient_color = tuple(c - c * i / num_points for c in color)
            cr.set_source_rgba(*gradient_color, transparency)
            cr.move_to(x1, height)
            cr.line_to(x2, height - bar_height * height)
            cr.fill()
...
#    def draw_waveform(self, cr, x, y, width, height, color):
    ...
    def init_gst(self):
        ...
        # Replace ALSA sink with PipeWire sink
    ...
    def read_audio_data(self):
        try:
            if self.stream_audio_input is None:
                # Jeśli strumień jest zamknięty, spróbuj ponownie go otworzyć
                self.stream_audio_input = self.pa.open(
                    format=pyaudio.paInt16,
                    channels=2,  # Stereo
                    rate=self.sample_rate,
            ...
            audio_data = self.stream_audio_input.read(self.chunk_size, exception_on_overflow=False)
            audio_data = np.frombuffer(audio_data, dtype=np.int16)
            return audio_data
        except IOError as e:
            print(f"Błąd odczytu danych audio: {e}")
    ...
    # Modify append_from_file to allow multiple file selection
    def append_from_file(self, widget):
        dialog = Gtk.FileChooserDialog("Please choose files", self.window, Gtk.FileChooserAction.OPEN,
                                       ("Cancel", Gtk.ResponseType.CANCEL, "Open", Gtk.ResponseType.OK))
        dialog.set_select_multiple(True)  # Allow selecting multiple files
