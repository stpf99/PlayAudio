...
class MusicPlayer:
    def __init__(self):
        ...
        self.sort_playlist_combobox = Gtk.ComboBoxText()
        self.sort_playlist_combobox.append_text("Sort By")
        self.sort_playlist_combobox.append_text("Title")
        self.sort_playlist_combobox.append_text("Artist")
        self.sort_playlist_combobox.append_text("Genre")
        self.sort_playlist_combobox.append_text("Album")
        self.sort_playlist_combobox.connect("changed", self.sort_playlist)

        hbox_buttons_top = Gtk.HBox(False, 0)
        hbox_buttons_top.pack_start(self.play_button, True, True, 2)
        hbox_buttons_top.pack_start(self.pause_button, True, True, 2)
        hbox_buttons_top.pack_start(self.stop_button, True, True, 2)
        hbox_buttons_top.pack_start(self.load_from_dir_button, True, True, 2)
        hbox_buttons_top.pack_start(self.append_from_file_button, True, True, 2)
        hbox_buttons_top.pack_start(self.sort_playlist_combobox, True, True, 2)
        
        hbox_buttons_bottom = Gtk.HBox(False, 0)
        hbox_buttons_bottom.pack_start(self.clear_playlist_button, True, True, 2)
        hbox_buttons_bottom.pack_start(self.shuffle_playlist_button, True, True, 2)
        hbox_buttons_bottom.pack_start(self.save_playlist_button, True, True, 2)
        hbox_buttons_bottom.pack_start(self.load_playlist_button, True, True, 2)
        hbox_buttons_bottom.pack_start(self.repeat_all_button, True, True, 2)
        hbox_buttons_bottom.pack_start(self.repeat_one_button, True, True, 2)
        hbox_buttons_bottom.pack_start(self.mute_button, True, True, 2)
        ...
        vbox.pack_start(hbox_buttons_top, False, False, 2)
        vbox.pack_start(scrolled_window, True, True, 2)
        vbox.pack_start(hbox_buttons_bottom, False, False, 2)
        vbox.pack_start(self.gain_scale, False, False, 2)
        vbox.pack_start(self.position_scale, False, False, 2)
        vbox.pack_start(self.time_label, False, False, 2)

        self.update_time_timer = GObject.timeout_add(1000, self.update_time_label)
        self.update_position_timer = GObject.timeout_add(1000, self.update_position_slider)
    ...
    def sort_playlist(self, widget):
        sort_column = widget.get_active_text()
        if sort_column == "Title":
            self.playlist_store.set_sort_column_id(0, Gtk.SortType.ASCENDING)
        elif sort_column == "Artist":
            self.playlist_store.set_sort_column_id(1, Gtk.SortType.ASCENDING)
        elif sort_column == "Genre":
            self.playlist_store.set_sort_column_id(2, Gtk.SortType.ASCENDING)
        elif sort_column == "Album":
            self.playlist_store.set_sort_column_id(3, Gtk.SortType.ASCENDING)
    ...
    def update_position_slider(self):
        if self.pipeline.get_state(0)[1] == Gst.State.PLAYING:
            ...
            if duration > 0:
                ...
                self.position_slider.set_value(position_percentage)
        return True

    def on_position_slider_change(self, widget):
        # Pobierz wartość suwaka pozycji i przelicz na czas
        value = self.position_slider.get_value()
        duration = self.pipeline.query_duration(Gst.Format.TIME)[1]
        seek_time = int(value * duration / 100)

        # Ustaw nową pozycję w odtwarzaczu
        self.pipeline.seek_simple(Gst.Format.TIME, Gst.SeekFlags.FLUSH, seek_time)

    def on_volume_scale_change(self, widget):
        # Pobierz wartość suwaka głośności i ustaw w odtwarzaczu
    ...
    def remove_selected(self, widget):
        ...
        if selected_song_iter:
            self.playlist_store.remove(selected_song_iter)

    def sort_playlist_title(self, widget):
        self.playlist_store.set_sort_column_id(4, Gtk.SortType.ASCENDING)

    def sort_playlist_artist(self, widget):
        self.playlist_store.set_sort_column_id(1, Gtk.SortType.ASCENDING)

    def sort_playlist_genre(self, widget):
        self.playlist_store.set_sort_column_id(2, Gtk.SortType.ASCENDING)

    def sort_playlist_album(self, widget):
        self.playlist_store.set_sort_column_id(3, Gtk.SortType.ASCENDING)
    ...
    def update_position_slider(self):
        if self.pipeline.get_state(0)[1] == Gst.State.PLAYING:
            ...
            if duration > 0:
                position_percentage = (position / duration) * 100
                self.position_scale.set_value(position_percentage)
            else:
