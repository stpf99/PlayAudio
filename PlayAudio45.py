...
class MusicPlayer:
    def __init__(self):
        ...
        GLib.timeout_add(250, self.update_spectrum)  # Aktualizacja spektrum co 100ms
    ...
    def draw_spectrum(self, widget, cr):
        ...
        x = width / 2 - 100
        y = height / 2
            # Pulsujący tekst "P L A Y    A U D I O"
        # Cieniowanie tekstu
        text = "P L A Y    A U D I O"
        cr.select_font_face("Sans", cairo.FONT_SLANT_NORMAL, cairo.FONT_WEIGHT_BOLD)
        cr.set_font_size(30)
        cr.set_font_options(self.font_options)
        
        cr.set_source_rgba(0, 0, 0, 0.6)  # Kolor cieniowania (czarny, 80% przezroczystości)
        cr.move_to(x + 2, y + 2)  # Przesunięcie cieniowania o 2 piksele w prawo i w dół
        cr.show_text(text)
        cr.fill()

        cr.set_source_rgba(1, 1, 1, self.text_opacity)  # Kolor tekstu (biały z przezroczystością)
        cr.move_to(x, y)
        cr.show_text(text)
        cr.stroke()
