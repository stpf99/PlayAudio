...
class MusicPlayer:
    def __init__(self):
        ...
        self.current_song_name = ""  # Dodaj nowy atrybut do przechowywania nazwy aktualnej piosenki
        ...
        # Create a fixed container to hold the album cover
        self.album_cover_fixed = Gtk.Fixed()
        self.album_cover_fixed.set_size_request(200, 200)  # Set the size of the album cover area
        ...
        self.visualization_type_combo.set_active(0)  # Domyślnie wybierz "Spectrum
        ...
        self.visualize_button.connect("clicked", self.visualize)
    ...
    def play(self, widget):
        ...
        if selected_song_iter:
            ...
            if self.repeat_current:
                self.buffer_audio_data(song_path)
            else:
    ...
    def play_audio_file(self, file_path):
        if self.stream_audio_output is not None:
            self.stream_audio_output.close()
            self.stream_audio_output = None
        ...
        if self.buffer_ready:
            try:
                ...
                self.stream_audio_output.write(audio_data.tobytes())
                self.stream_audio_output.close()
            except Exception as e:
                print(f"Error while playing audio from buffer: {e}")
                self.buffer_ready = False
        else:
            try:
                encoded_file_path = quote(file_path)
