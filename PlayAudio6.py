...
class MusicPlayer:
    def __init__(self):
        ...
        self.add_playlist_column("Title", 4)
        self.add_playlist_column("Artist", 1)
        self.add_playlist_column("Genre", 2)
        self.add_playlist_column("Album", 3)
        self.add_playlist_column("File Path", 0)
        self.add_playlist_column("Stats", 6)
        # Connect sort functionality to columns
        for column_id in range(5):
            self.playlist_view.get_column(column_id).connect("clicked", self.sort_column_clicked, column_id)
        ...
        self.stats = {}  # Słownik do przechowywania statystyk odtwarzania
        ...
        # Odczytaj wcześniejsze statystyki z pliku, jeśli istnieją
        self.load_stats_from_file()


    def create_playlist_view(self):
        # Create the TreeView and set it to the ListStore
        self.playlist_view = Gtk.TreeView(self.playlist_store)
        self.playlist_view.set_rules_hint(True)
        self.playlist_view.set_headers_clickable(True)  # Allow sorting by clicking column headers

        # Add columns for Title, Artist, Genre, Album, and Stats
        self.add_playlist_column("Title", 4)  # Dodaj kolumnę tytułów
        self.add_playlist_column("Artist", 1)
        self.add_playlist_column("Genre", 2)
        self.add_playlist_column("Album", 3)
        self.add_playlist_column("Stats", 5)  # New column for song statistics

        # Set the default sorting column and order
        self.playlist_view.get_model().set_sort_column_id(0, Gtk.SortType.ASCENDING)

        # Connect the "clicked" signal to handle column header clicks for sorting
        self.playlist_view.get_column(0).connect("clicked", self.sort_column_clicked, 0)
        self.playlist_view.get_column(1).connect("clicked", self.sort_column_clicked, 1)
        self.playlist_view.get_column(2).connect("clicked", self.sort_column_clicked, 2)
        self.playlist_view.get_column(3).connect("clicked", self.sort_column_clicked, 3)
        self.playlist_view.get_column(4).connect("clicked", self.sort_column_clicked, 4)
        self.playlist_view.get_column(5).connect("clicked", self.sort_column_clicked, 5)
    ...
    def play_audio_file(self, file_path):
        try:
            ...
            # Zaktualizuj statystyki odtwarzania
            if file_path in self.stats:
                self.stats[file_path]["play_count"] += 1
            else:
    ...
    def on_position_scale_change(self, widget):
        if self.pipeline.get_state(0)[1] == Gst.State.PLAYING:
            ...
            if selected_song_iter:
                song_path = model.get_value(selected_song_iter, 0)
                if song_path in self.stats:
                    if value <= 25:
                        self.stats[song_path]["25_percent"] += 1
                    elif value <= 50:
                        self.stats[song_path]["50_percent"] += 1
                    elif value <= 75:
                        self.stats[song_path]["75_percent"] += 1
                    else:
                        self.stats[song_path]["100_percent"] += 1

    def save_stats_to_file(self):
        try:
            with open("/home/tomek/Dokumenty/stats.txt", "w") as f:
                for song_path, stat in self.stats.items():
                    play_count = stat["play_count"]
                    count_25_percent = stat["25_percent"]
                    count_50_percent = stat["50_percent"]
                    count_75_percent = stat["75_percent"]
                    count_100_percent = stat["100_percent"]

                    f.write(f"[{song_path}];[{count_25_percent}];[{count_50_percent}];[{count_75_percent}];[{count_100_percent}]\n")
    ...
    def load_stats_from_file(self):
        try:
            self.stats = {}  # Wyczyść istniejące statystyki
            ...
            if os.path.exists("stats.txt"):
                with open("stats.txt", "r") as f:
                    ...
                    for line in lines:
                        ...
                        if len(parts) == 5:
                            song_path, count_25_percent, count_50_percent, count_75_percent, count_100_percent = parts
                            self.stats[song_path] = {
                                "play_count": int(count_25_percent),
                                "25_percent": int(count_50_percent),
                                "50_percent": int(count_75_percent),
                                "75_percent": int(count_100_percent),
                                "100_percent": int(count_100_percent),
                            }

        except Exception as e:
            print(f"Error while loading stats: {e}")
    ...
    def add_playlist_column(self, title, column_id):
        renderer = Gtk.CellRendererText()
        column = Gtk.TreeViewColumn(title, renderer, text=column_id)
        column.set_sort_column_id(column_id)
        self.playlist_view.append_column(column)

    def sort_column_clicked(self, column, column_id):
        self.playlist_store.set_sort_column_id(column_id, Gtk.SortType.ASCENDING)
